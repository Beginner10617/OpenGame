...
Map::Map(std::map<std::pair<int, int>, char> &map, const char *atlasTexture,
  ...
  float x0, x1, z0, z1;
  Tile tileI;
  glm::vec2 tileuvMin, tileuvMax;
  unsigned int i = 0;
  for (auto &[coord, tile] : map) {
    x0 = (coord.first - 0.5f) * scale;
    x1 = (coord.first + 0.5f) * scale;
    z0 = (coord.second - 0.5f) * scale;
    z1 = (coord.second + 0.5f) * scale;
    tileI = tileInfo[tile];
    tileuvMin = tileI.uvMin;
    tileuvMax = tileI.uvMax;
    vData.insert(vData.end(), {x0, groundy, z0, tileuvMin.x, tileuvMin.y,
    ...
    iData.insert(iData.end(),
                 {4 * i, 4 * i + 1, 4 * i + 2, 4 * i, 4 * i + 3, 4 * i + 2});
    i++;
  }
...
Map::Map(const char *filepath, float groundy, float scale) {
  ...
  while (std::getline(file, line)) {
    ...
    if (curr_section == MAPDATA) {
      ...
      if (texH == 0 || texW == 0) {
        printf(
            "Incorrect formatting, declare atlas dimension before tileInfo\n");
        exit(EXIT_FAILURE);
      }
      size_t count = 0;
      std::string tmp = "";
      ...
      if (tmp != "")
        y = atoi(tmp.c_str());
      tileInfo[tname] = {
          glm::vec2((float)x / (float)texW, (float)y / (float)texH),
          glm::vec2((float)(x + 1) / (float)texW,
                    (float)(y + 1) / (float)texH)};
      continue;
  ...
  *this = Map(map, atlasTexture.c_str(), tileInfo, groundy, scale);
}
